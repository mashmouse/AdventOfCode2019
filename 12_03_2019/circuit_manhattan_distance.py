...
def find_map_shape_from_origin(lst):
    ...
    for instruction in lst:
        ...
        if instruction.startswith('R'):
            x_pos += num
            r_bound = max(r_bound, x_pos)
        elif instruction.startswith('L'):
            x_pos -= num
            l_bound = min(l_bound, x_pos)
        elif instruction.startswith('U'):
            y_pos -= num
            u_bound = min(u_bound, y_pos)
        elif instruction.startswith('D'):
            y_pos += num
            d_bound = max(d_bound, y_pos)
    width = r_bound + abs(l_bound)
    height = abs(u_bound) + l_bound
    return {'height': height, 'width': width, 'y_origin': abs(u_bound), 'x_origin' : abs(l_bound)}

def find_two_map_shape(lst1, lst2):
    map_shape_info_1 = find_map_shape_from_origin(lst1)
    map_shape_info_2 = find_map_shape_from_origin(lst2)
    print(map_shape_info_1)
    print(map_shape_info_2)
    # Merge map size data
    #return dict(map(lambda kv1, kv2: (kv1[0], max(kv1[1], kv2[1])), map_shape_info_1.items(), map_shape_info_2.items()))
    new_y_origin = max(map_shape_info_1['y_origin'], map_shape_info_2['y_origin'])
    new_x_origin = max(map_shape_info_1['x_origin'], map_shape_info_2['x_origin'])
    new_height = new_y_origin + max(map_shape_info_1['height'] - map_shape_info_1['y_origin'], map_shape_info_2['height'] - map_shape_info_2['y_origin'])
    new_width = new_x_origin + max(map_shape_info_1['width'] - map_shape_info_1['x_origin'], map_shape_info_2['width'] - map_shape_info_2['x_origin'])
    return {'height': new_height,'width': new_width, 'y_origin': new_y_origin, 'x_origin': new_x_origin}

def trace_wire(block_map, lst, y_origin, x_origin):
    x_pos = x_origin
    y_pos = y_origin
    ...
    for instruction in lst:
        ...
        if instruction.startswith('R'):
            new_x_pos = x_pos + num
            block_map[y_pos,x_pos:new_x_pos + 1] += 1
            x_pos = new_x_pos
            #block_map[y_pos, x_pos] = -1
        elif instruction.startswith('L'):
            new_x_pos = x_pos - num
            block_map[y_pos,new_x_pos:x_pos] += 1
            x_pos = new_x_pos
        elif instruction.startswith('U'):            
            new_y_pos = y_pos - num
            block_map[new_y_pos:y_pos + 1, x_pos] += 1
            y_pos = num
        elif instruction.startswith('D'):
            new_y_pos = y_pos + num
            block_map[y_pos:new_y_pos, x_pos] += 1
            y_pos = num
    return block_map

def find_nearest_intersection(block_map, height, width):
    intersections = []
...
def manhattan_distance(lst1, lst2):   
    map_info = find_two_map_shape(lst1, lst2)
    block_map = np.zeros((map_info['width'],map_info['height']))
    
    block_map = trace_wire(block_map, ['R6', 'U6'], map_info['y_origin'], map_info['x_origin'])
    print(block_map)
    #block_map = trace_wire(block_map, lst2, map_info['x_origin'], map_info['y_origin'])

    

manhattan_distance(sample_z_1, sample_z_2)
#m = np.zeros((2,2))
#print(find_two_map_shape(sample_a_1, sample_a_2))
